...
''', encoding='utf8')
        ...
        with sync_playwright() as pw:
            ...
            auto.uncheck();saved=viewport_state()
            body.dispatch_event('scroll');expect(auto).not_to_be_checked()
            check('Explicit disable is not undone by a late scroll event at bottom')
            emit(800,False);page.wait_for_timeout(150)
            ...
            page.locator('.sidebar .nav-item').filter(has_text='项目管理').click()
            emit(960,False)
            page.locator('.sidebar .nav-item').filter(has_text='运行台').click()
