...
def main():
 with Native() as n:
  ...
  with sync_playwright() as pw:
   ...
   page.get_by_label('移动到分组').select_option(group['id']);page.get_by_role('button',name='移入',exact=True).click();expect(page.locator('.live-file')).to_have_count(1);expect(page.locator('.live-file')).to_contain_text('work.txt');check('Batch move removes protected file from default group')
   ...
   page.locator('.live-commit textarea').fill('default group UI commit');page.get_by_role('button',name='检查并提交「默认」',exact=True).click();expect(page.locator('.live-confirm-files')).to_have_text('work.txt');check('Commit review only includes current group')
