...
export function useWorkbenchV04(ctx){
 ...
 function configForm(c){
  ...
  form.value=c?{id:c.id,name:c.name,cwd:c.cwd,output_encoding:c.output_encoding||'auto',environment_id:c.environment_id||'',launcher:{...c.launcher},argumentRows:asRows(c.launcher.arguments),vmRows:asRows(c.launcher.vm_options),propertyRows:asRows(c.launcher.properties),envRows:asRows(c.env),profileRows:asRows(c.launcher.profiles),mavenProfileRows:asRows(c.launcher.maven_profiles),mavenPropertyRows:asRows(c.launcher.maven_properties),watchRows:asRows(c.watch)}:{id:'',name:'',cwd:'.',output_encoding:'auto',environment_id:'',launcher:null,argumentRows:[],vmRows:[],propertyRows:[],envRows:[],profileRows:[],mavenProfileRows:[],mavenPropertyRows:[],watchRows:[]}
 ...
 async function pickLauncherDirectory(field){await act(async()=>{const p=(await api('fs.pick',{kind:'folder',directory:project.value.root})).path;if(p){form.value.launcher[field]=(await api('project.relative',{project:project.value.id,path:p})).path;commandPreview.value=null;}})}
