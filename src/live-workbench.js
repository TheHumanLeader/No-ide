...
export function createLiveWorkbench(health) {
 return {
  ...
  setup() {
   ...
   function configForm(c){form.value=c?{id:c.id,name:c.name,cwd:c.cwd,output_encoding:c.output_encoding||'auto',program:c.command.program,args:c.command.args.join('\n'),buildProgram:c.build?.program||'',buildArgs:(c.build?.args||[]).join('\n'),watch:c.watch.join('\n'),env:Object.entries(c.env).map(([k,v])=>`${k}=${v}`).join('\n')}:{id:'',name:'',cwd:'.',output_encoding:'auto',program:'',args:'',buildProgram:'',buildArgs:'',watch:'',env:''};modal.value='config'}
   ...
   async function saveConfig(){await act(async()=>{const f=form.value;await api('config.save',{project:project.value.id,config:{id:f.id,name:f.name,cwd:f.cwd,output_encoding:f.output_encoding||'auto',command:{program:f.program,args:lines(f.args)},build:f.buildProgram?{program:f.buildProgram,args:lines(f.buildArgs)}:null,watch:lines(f.watch),env:envParse(f.env)}});await refresh();modal.value='';notice.value='运行配置已保存。下一步添加一个或多个实例。'})}
