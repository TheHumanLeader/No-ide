...
export function createLiveWorkbench(health) {
 return {
  ...
  setup() {
   ...
   async function selectFile(f){file.value=f.path;await loadDiff()}
   async function loadDiff(){if(!file.value||!repo.value)return;const epoch=++diffEpoch,p=project.value.id,r=repoId.value,path=file.value;await act(async()=>{const d=await api('vcs.diff',{project:p,repo:r,path,staged:staged.value});if(epoch===diffEpoch)diff.value=d.diff})}
   ...
   const opName=o=>({stage:'暂存选中文件',unstage:'取消暂存',commit:repo.value?.kind==='svn'?'提交到 SVN 服务器':'提交到本地 Git 仓库',pull:'拉取 origin（仅快进）',push:'推送到 origin',update:'更新 SVN 工作副本',add:'添加到 SVN 版本管理'}[o]||o)
