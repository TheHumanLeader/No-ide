...
pub struct Groups{pub items:Vec<Group>,pub assignments:BTreeMap<String,String>}
impl Default for Groups{fn default()->Self{Self{items:vec![Group{id:DEFAULT.into(),name:"默认".into()}],assignments:BTreeMap::new()}}}
impl Groups{
 ...
 pub fn save(&mut self,group_id:&str,name:&str)->Result<Group>{
  ...
  if group_id.is_empty(){if self.items.len()>=64{return fail("每个仓库最多 64 个分组")};let g=Group{id:id(),name:name.into()};self.items.push(g.clone());Ok(g)}
  else{let g=self.items.iter_mut().find(|g|g.id==group_id).ok_or_else(||Error("分组不存在".into()))?;g.name=name.into();Ok(g.clone())}
 }
 pub fn remove(&mut self,id:&str)->Result<()>{if id==DEFAULT{return fail("默认分组不可删除")};self.name(id)?;self.items.retain(|g|g.id!=id);self.assignments.retain(|_,g|g!=id);Ok(())}
 pub fn set(&mut self,paths:&[String],id:&str)->Result<()>{self.name(id)?;if self.assignments.len()+paths.iter().filter(|p|!self.assignments.contains_key(*p)).count()>20000{return fail("已记忆的文件分组达到 20000 条上限")};for p in paths{self.assignments.insert(p.clone(),id.into());}Ok(())}
}
#[cfg(test)]mod tests{
 use super::*;
