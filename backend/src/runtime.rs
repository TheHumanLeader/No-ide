...
impl Runtime{
 ...
 async fn actor(self,project:Project,cfg:RunConfig,mut store:Store,mut rx:mpsc::Receiver<Control>,cancel:watch::Sender<u64>){
  let root=project.root.clone();let deleted=Arc::new(AtomicBool::new(false));let mut watch_paths=cfg.watch.clone();
  if !watch_paths.is_empty(){if let Some(l)=&cfg.launcher{if ["spring-maven","maven-main"].contains(&l.kind.as_str()){
   match inside(&root,&cfg.cwd).and_then(|cwd|crate::launch::maven::resolve(&root,&cwd,l)).and_then(|plan|plan.watch_paths(&root)){Ok(paths)=>watch_paths.extend(paths),Err(e)=>self.hub.log(&cfg.id,"error",&format!("依赖模块监听未扩展：{}",e.0)).await,}
  }}}
...
async fn log_chunks(hub:&Hub,id:&str,stream:&str,text:&str){let mut chunk=String::new();let mut count=0;for ch in text.chars(){chunk.push(ch);count+=1;if count==4096{hub.log(id,stream,&chunk).await;chunk.clear();count=0;}}if !chunk.is_empty(){hub.log(id,stream,&chunk).await;}}
async fn pump<R:AsyncRead+Unpin>(mut r:R,hub:Arc<Hub>,id:String,stream:&str,encoding:crate::process::text_output::OutputEncoding){
 let mut decoder=crate::process::text_output::TextDecoder::new(encoding);let mut warned=false;let mut b=[0u8;4096];loop{let n=match r.read(&mut b).await{Ok(n)=>n,Err(e)=>{hub.log(&id,"error",&format!("读取 {stream} 日志失败：{e}")).await;break}};let piece=decoder.push(&b[..n],n==0);if piece.replacements&&!warned{warned=true;hub.log(&id,"encoding",&crate::process::text_output::warning(piece.encoding)).await;}if !piece.text.is_empty(){log_chunks(&hub,&id,stream,&piece.text).await;}if n==0{break}}
}
...
async fn build_pipe<R:AsyncRead+Unpin>(mut r:R,hub:Arc<Hub>,id:String,encoding:crate::process::text_output::OutputEncoding)->Result<String>{
 ...
 loop{let n=r.read(&mut b).await?;let part=decoder.push(&b[..n],n==0);if part.replacements&&!warned{warned=true;hub.log(&id,"encoding",&crate::process::text_output::warning(part.encoding)).await;}log_chunks(&hub,&id,"build",&part.text).await;tail.push_str(&part.text);if tail.len()>32768{let mut cut=tail.len()-32768;while !tail.is_char_boundary(cut){cut+=1;}tail.drain(..cut);}if n==0{break}}Ok(tail)
}
pub(crate) async fn capture_build(c:tokio::process::Command,seconds:u64,hub:Arc<Hub>,id:String,encoding:crate::process::text_output::OutputEncoding)->Result<process::Output>{
 let mut child=process::spawn(c)?;let out=child.0.stdout().take().ok_or_else(||Error("构建标准输出未连接".into()))?;let err=child.0.stderr().take().ok_or_else(||Error("构建错误输出未连接".into()))?;
 let result=timeout(Duration::from_secs(seconds),async{let(status,stdout,stderr)=tokio::try_join!(async{child.0.wait().await.map_err(Error::from)},build_pipe(out,hub.clone(),id.clone(),encoding),build_pipe(err,hub,id,encoding))?;Ok::<_,Error>(process::Output{code:status.code().unwrap_or(-1),stdout,stderr,warnings:vec![]})}).await;
 let _=child.0.start_kill();let _=timeout(Duration::from_secs(3),child.0.wait()).await;result.map_err(|_|Error("构建超时，已请求停止受控构建进程；未启动新实例".into()))?
}
