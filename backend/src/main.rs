mod core;mod process;mod tools;mod runtime;mod scm;
use crate::core::*;
...
async fn call(State(s):State<Arc<App>>,Json(call):Json<Call>)->Result<Json<Value>>{
 let v=&call.params;let result=match call.action.as_str(){
  ...
  "project.add"=>{
   ...
   let mut repos=vec![];for(kind,meta)in[("git",".git"),("svn",".svn")]{if root.join(meta).exists(){repos.push(Repo{id:id(),kind:kind.into(),path:root.clone()});}}
  ...
  "config.save"|"instance.save"|"repo.add"=>{
   ...
   let object=match call.action.as_str(){
    ...
    "instance.save"=>{let mut i:Instance=val(&v["instance"])?;if i.id.is_empty(){i.id=id()};text(&i.name,100)?;validate_env(&i.env)?;if i.args.len()>128||i.args.iter().any(|a|a.contains('\0')||a.len()>8192){return fail("实例参数无效")};if !p.configs.iter().any(|c|c.id==i.config_id){return fail("运行配置不存在")};if let Some(port)=i.port{if port==0||all_ports.iter().any(|(id,p)|*p==port&&*id!=i.id){return fail("端口为 0 或已被其他实例配置占用")}}
  ...
  "run.start"|"run.stop"|"run.update"=>{
   ...
   if call.action=="run.start"{s.runtime.start(p,i).await?}else{s.runtime.control(&p,&i,if call.action=="run.stop"{"stop"}else{"update"}).await?};json!({"accepted":true})
  },
  "vcs.status"|"vcs.diff"|"vcs.prepare"|"vcs.execute"=>{
   let _guard=s.vcs.lock().await;let _operation=s.writes.lock().await;let store=s.store.lock().await.clone();let p=store.project(string(v,"project")?)?;let r=p.repos.iter().find(|r|Some(r.id.as_str())==v["repo"].as_str()).ok_or_else(||Error("仓库不存在".into()))?;
   let client=scm::Client::new(&store,&p,r).await?;
   match call.action.as_str(){
    "vcs.status"=>json!(client.status().await?),
    "vcs.diff"=>json!({"diff":client.diff(string(v,"path")?,v["staged"].as_bool().unwrap_or(false)).await?}),
    "vcs.prepare"=>{let operation=string(v,"operation")?;if ["pull","update"].contains(&operation)&&s.runtime.project_active(&p).await{return fail("请先停止本项目实例再更新仓库，防止运行半更新代码")};let paths=val::<Vec<String>>(&v["paths"]).unwrap_or_default();let plan=s.plans.lock().await.prepare(&client,&p.id,operation,paths,v["message"].as_str().unwrap_or("")).await?;json!(plan)},
...
fn entry()->Result<()>{
 ...
 if args.iter().any(|a|a=="--pick-folder"||a=="--pick-file"){
  let d=rfd::FileDialog::new().set_title("No-ide · 选择本机目录或程序");let p=if args.iter().any(|a|a=="--pick-file"){d.pick_file()}else{d.pick_folder()};
