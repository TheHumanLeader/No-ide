 ...
 let result=match call.action.as_str(){
  ...
  "vcs.status"|"vcs.diff"|"vcs.prepare"|"vcs.execute"=>{
   ...
   match call.action.as_str(){
    "vcs.status"=>{let status=client.status().await?;let mut output=json!(&status);output["groups"]=json!(r.groups.items);for (row,f) in output["files"].as_array_mut().unwrap().iter_mut().zip(status.files.iter()){row["group"]=json!(r.groups.of(&f.path,f.original.as_deref()));}output},
